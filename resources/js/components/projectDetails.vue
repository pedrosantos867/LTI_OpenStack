<template>
  <div>
    ...
    <button class="btn btn-primary" v-on:click="createInstance()">Criar nova instância</button>
    <br />
    <br />
    <br />
        ...
        <table class="table table-striped">
          ...
          <tbody>
            <tr v-for="instance in instancesDetails" :key="instance">
              ...
              <td>
                <div
                  v-for="privateIP in instance.addresses.private"
                  :key="privateIP"
                >{{privateIP.addr}}</div>
                <div v-for="sharedIP in instance.addresses.shared" :key="sharedIP">{{sharedIP.addr}}</div>
                <div v-for="publicIP in instance.addresses.public" :key="publicIP">{{publicIP.addr}}</div>
              </td>
              ...
              <td>
                <div v-if="instance.locked">{{"LOCKED"}}</div>
                <div v-else>{{"NOT LOCKED"}}</div>
              </td>
...
export default {
  data: function() {
    return {
      title:
        "Lista de instancias do projeto " +
        this.$store.state.currentProjectName,
      instancesList: [],
      instancesDetails: [],
      imagesDetails: [],
      flavorsDetails: []
  ...
  methods: {
    getInstances: function() {
      axios
        .get(this.$store.state.url + "/compute/v2.1/servers", {
          headers: {
    ...
    removeInstance: function(instance) {
      ...
      axios
        .delete(
          this.$store.state.url + "/compute/v2.1/servers/" + instance.id,
          {
            headers: {
              "Content-Type": "application/json",
              "X-Auth-Token": this.$store.state.projectScopedToken
        ...
        .then(response => {
          if (response.status == 204) {
            Vue.$toast.open(
              "Instância " + instance.name + " apagado com sucesso!"
            );
            this.instancesList = [];
            this.getInstances();
          } else {
            Vue.$toast.open(
              "Houve um erro ao apagar a instância " + instance.name + "!"
            );
          }
